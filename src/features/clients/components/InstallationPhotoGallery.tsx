...
import { AlertTriangle, Camera, Image as ImageIcon, LoaderCircle, MapPin, Pencil, Save, Star, Trash2, Upload, X } from 'lucide-react';
...
export default function InstallationPhotoGallery({ tenantId, installationId, installationName, address, contactName, contactPhone, canManage, client }: Props) {
  ...
  const editMutation = useMutation({
    mutationFn: () => {
      if (!editDraft) throw new Error('Selecciona una fotografía para editar.');
      return updateInstallationPhotoMetadata(supabase, editDraft.id, editDraft);
    },
    onSuccess: async () => { setEditDraft(null); await queryClient.invalidateQueries({ queryKey }); },
  });
  ...
  const deleteMutation = useMutation({ mutationFn: (photo: InstallationPhoto) => deleteInstallationPhoto(supabase, photo), onSuccess: async () => { setConfirmDelete(null); setEditDraft(null); await queryClient.invalidateQueries({ queryKey }); } });
  ...
  const beginEdit = (photo: InstallationPhoto) => setEditDraft({ id: photo.id, title: photo.title || '', description: photo.description || '', category: photo.category });
  ...
  return <section className="installation-gallery" aria-labelledby={`installation-gallery-${installationId}`}>
    ...
    {photos.length > 0 && <div className="installation-photo-grid">{photos.map((photo) => <article className={photo.main ? 'is-main' : ''} key={photo.id}><div>{photo.signedUrl ? <img alt={photo.title || photo.filename} loading="lazy" src={photo.signedUrl} /> : <span><AlertTriangle size={18} /> Sin vista</span>}</div>{editDraft?.id === photo.id ? <div className="installation-photo-edit"><label><span>Título</span><input maxLength={160} onChange={(event) => setEditDraft({ ...editDraft, title: event.target.value })} value={editDraft.title} /></label><label><span>Descripción</span><textarea maxLength={1000} onChange={(event) => setEditDraft({ ...editDraft, description: event.target.value })} rows={2} value={editDraft.description} /></label><label><span>Categoría</span><select onChange={(event) => setEditDraft({ ...editDraft, category: event.target.value as InstallationPhotoCategory })} value={editDraft.category}>{Object.entries(categoryLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><div className="form-actions"><button className="secondary-button" onClick={() => setEditDraft(null)} type="button"><X size={15} /> Cancelar</button><button className="primary-button" disabled={editMutation.isPending} onClick={() => editMutation.mutate()} type="button">{editMutation.isPending ? <LoaderCircle className="spin" size={15} /> : <Save size={15} />} Guardar</button></div></div> : <p><span>{categoryLabels[photo.category]}</span><strong>{photo.title || photo.filename}</strong><small>{photo.description || 'Sin descripción'}</small></p>}{canManage && editDraft?.id !== photo.id && <footer><button aria-label={`Editar ${photo.title || photo.filename}`} className="icon-button" onClick={() => beginEdit(photo)} type="button"><Pencil size={16} /></button>{!photo.main && <button className="secondary-button" disabled={mainMutation.isPending} onClick={() => mainMutation.mutate(photo)} type="button"><Star size={15} /> Principal</button>}{confirmDelete === photo.id ? <><button className="secondary-button" onClick={() => setConfirmDelete(null)} type="button">Cancelar</button><button className="danger-button" disabled={deleteMutation.isPending} onClick={() => deleteMutation.mutate(photo)} type="button">Confirmar</button></> : <button aria-label={`Eliminar ${photo.title || photo.filename}`} className="icon-button" onClick={() => setConfirmDelete(photo.id)} type="button"><Trash2 size={16} /></button>}</footer>}</article>)}</div>}
    ...
    {(selectionError || uploadMutation.error || editMutation.error || mainMutation.error || deleteMutation.error) && <p className="form-global-error" role="alert"><AlertTriangle size={16} /> {selectionError || errorMessage(uploadMutation.error || editMutation.error || mainMutation.error || deleteMutation.error)}</p>}
  </section>;
}
