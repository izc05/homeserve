...
import ClientLogoEditor from '../components/ClientLogoEditor';
...
function displayActivity(value: string | null | undefined) {
...
export default function ClientsWorkspace({ tenantId, canManage, onCreateWorkOrder }: ClientsWorkspaceProps) {
  ...
  const clientsQuery = useQuery({ queryKey: ['clients', tenantId], queryFn: () => clientsService.list(tenantId), enabled: Boolean(tenantId) });
  const detailQuery = useQuery({ queryKey: ['client-detail', tenantId, selectedClientId], queryFn: () => clientsService.detail(tenantId, selectedClientId ?? ''), enabled: Boolean(tenantId && selectedClientId) });
  ...
  const createClientMutation = useMutation({ mutationFn: (values: ClientFormValues) => clientsService.create({ tenantId, ...values }), onSuccess: async (client) => { await refreshClientData(); setSelectedClientId(client.id); setClientEditor(null); } });
  const updateClientMutation = useMutation({ mutationFn: (values: ClientFormValues) => clientsService.update(tenantId, selectedClientId ?? '', values), onSuccess: async () => { await refreshClientData(); setClientEditor(null); } });
  const updateClientStatusMutation = useMutation({ mutationFn: ({ clientId, nextStatus }: { clientId: string; nextStatus: EntityStatus }) => clientsService.setStatus(tenantId, clientId, nextStatus), onSuccess: refreshClientData });
  const createInstallationMutation = useMutation({ mutationFn: (values: InstallationFormValues) => clientsService.createInstallation({ tenantId, ...values }), onSuccess: async () => { await refreshClientData(); setInstallationEditor(null); } });
  const updateInstallationMutation = useMutation({ mutationFn: ({ installationId, values }: { installationId: string; values: InstallationFormValues }) => clientsService.updateInstallation(tenantId, installationId, values), onSuccess: async () => { await refreshClientData(); setInstallationEditor(null); } });
  const updateInstallationStatusMutation = useMutation({ mutationFn: ({ installation, nextStatus }: { installation: ClientInstallation; nextStatus: EntityStatus }) => clientsService.setInstallationStatus(tenantId, installation, nextStatus), onSuccess: refreshClientData });

  const clients = useMemo(() => clientsQuery.data ?? [], [clientsQuery.data]);
  const filteredClients = useMemo(() => filterClients(clients, search, status), [clients, search, status]);
  const detail = detailQuery.data;
  const selectedInstallation = installationEditor && installationEditor !== 'create' ? detail?.installations.find((item) => item.id === installationEditor) ?? null : null;
  const installationDetail = selectedInstallationId ? detail?.installations.find((item) => item.id === selectedInstallationId) ?? null : null;

  const saveNewClient = async (values: ClientFormValues) => { try { await createClientMutation.mutateAsync(values); } catch (error) { throw new Error(friendlyClientError(error, 'No se pudo crear el cliente.')); } };
  const saveClient = async (values: ClientFormValues) => { try { await updateClientMutation.mutateAsync(values); } catch (error) { throw new Error(friendlyClientError(error, 'No se pudo actualizar el cliente.')); } };
  ...
  const toggleClient = (client: ClientRecord) => {
    ...
    if (window.confirm(`${nextStatus === 'activo' ? 'Activar' : 'Desactivar'} a ${client.name}? El historial se conservará.`)) updateClientStatusMutation.mutate({ clientId: client.id, nextStatus });
  };
  const toggleInstallation = (installation: ClientInstallation) => {
    ...
    if (window.confirm(`${nextStatus === 'activo' ? 'Activar' : 'Desactivar'} ${installation.name}?`)) updateInstallationStatusMutation.mutate({ installation, nextStatus });
  };
  ...
  if (selectedClientId) {
    ...
    const lastActivity = detail.recentWorkOrders[0]?.updatedAt ?? detail.client.updatedAt;

    return <section className="clients-workspace">
      <div className="page-heading client-heading client-heading-premium">
        <button className="text-link" onClick={() => { setSelectedClientId(null); setClientEditor(null); setInstallationEditor(null); setSelectedInstallationId(null); }} type="button"><ArrowLeft size={16} /> Clientes</button>
        <div className="client-identity-heading"><ClientLogoEditor client={detail.client} canManage={canManage} /><div><span className="section-kicker">Ficha de cliente</span><div className="client-title-row"><div><h1>{detail.client.name}</h1><p>{detail.client.code || 'Sin código'}{detail.client.cifNif ? ` · ${detail.client.cifNif}` : ''}</p></div><span className={statusClass(detail.client.status)}>{statusLabel(detail.client.status)}</span></div></div></div>
      </div>
      ...
      <div className="client-detail-grid client-detail-grid-balanced">
        ...
        <section className="panel client-summary-panel"><div className="panel-heading"><h2>Resumen</h2></div><div className="client-stat"><Building2 size={19} /><span><strong>{detail.installations.length}</strong> instalaciones</span></div><div className="client-stat"><ClipboardList size={19} /><span><strong>{detail.openWorkOrders.length}</strong> OT abiertas</span></div><div className="client-stat"><ClipboardList size={19} /><span><strong>{detail.recentWorkOrders.length}</strong> OT recientes</span></div><div className="client-stat"><RefreshCw size={19} /><span><strong>Última actividad</strong><small>{displayActivity(lastActivity)}</small></span></div></section>
      </div>

      <section className="client-section">
        <div className="section-row"><div><h2>Instalaciones</h2><p>Centros y ubicaciones operativas de {detail.client.name}.</p></div>{canManage && <button className="secondary-button" onClick={() => setInstallationEditor(installationEditor === 'create' ? null : 'create')} type="button"><Plus size={17} /> Nueva instalación</button>}</div>
        {clientHasNoInstallations ? <section className="panel client-empty-state"><Building2 size={28} /><strong>Este cliente no tiene instalaciones</strong><p>Antes de crear una OT debes registrar al menos una instalación activa.</p>{canManage && <button className="primary-button" onClick={() => setInstallationEditor('create')} type="button"><Plus size={17} /> Crear instalación</button>}</section> : <div className="installation-list">{detail.installations.map((installation) => {
          const directions = workOrderDirectionsUrl({ address: installation.address, latitude: installation.latitude, longitude: installation.longitude });
          return <article className={`installation-row ${selectedInstallationId === installation.id ? 'is-selected' : ''}`} key={installation.id}><div className="installation-icon"><Building2 size={20} /></div><div><strong>{installation.code ? `${installation.code} · ` : ''}{installation.name}</strong><small>{installation.type || 'Tipo sin definir'}{installation.address ? ` · ${installation.address}` : ''}</small><small>{installation.contactName || 'Sin contacto'}{installation.contactPhone ? ` · ${installation.contactPhone}` : ''}</small></div><span className={statusClass(installation.status)}>{statusLabel(installation.status)}</span><div className="row-actions">{directions && <a className="icon-button" aria-label={`Cómo llegar a ${installation.name}`} href={directions} onClick={(event) => event.stopPropagation()} rel="noopener noreferrer" target="_blank"><MapPin size={16} /></a>}<button className="secondary-button installation-open-button" aria-expanded={selectedInstallationId === installation.id} onClick={() => setSelectedInstallationId((current) => current === installation.id ? null : installation.id)} type="button"><Eye size={16} /> {selectedInstallationId === installation.id ? 'Cerrar ficha' : 'Abrir ficha'}</button>{canManage && <><button className="icon-button" aria-label={`Editar ${installation.name}`} onClick={() => setInstallationEditor(installation.id)} type="button"><Edit3 size={16} /></button><button className="icon-button" aria-label={`${installation.status === 'activo' ? 'Desactivar' : 'Activar'} ${installation.name}`} onClick={() => toggleInstallation(installation)} type="button"><Power size={16} /></button></>}</div></article>;
        })}</div>}
        ...
        {installationDetail && <div className="installation-expanded-detail"><LocationMapCard address={installationDetail.address} latitude={installationDetail.latitude} longitude={installationDetail.longitude} installationName={installationDetail.name} /><InstallationPhotoGallery tenantId={tenantId} installationId={installationDetail.id} installationName={installationDetail.name} address={installationDetail.address} contactName={installationDetail.contactName} contactPhone={installationDetail.contactPhone} canManage={canManage} /></div>}
      </section>
