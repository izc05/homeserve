...
describe('toCreateWorkOrderRpcArgs', () => {
  ...
  it('envía los requisitos y el sistema opcional en el sobre compatible', () => {
    const args = toCreateWorkOrderRpcArgs(baseInput);
  ...
  it('transporta un sistema técnico explícito sin cambiar la firma RPC', () => {
    const systemId = '44444444-4444-4444-8444-444444444444';
    const args = toCreateWorkOrderRpcArgs({ ...baseInput, systemId });

    expect(args).not.toHaveProperty('system_uuid');
    expect(args.requirements_json).toMatchObject({ sistema_id: systemId });
  });
