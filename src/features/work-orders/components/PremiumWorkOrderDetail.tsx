      ...
      <div className="premium-tab-panel" id={`premium-panel-${activeTab}`} role="tabpanel" aria-labelledby={`premium-tab-${activeTab}`} tabIndex={0}>
        {activeTab === 'summary' && <>
          <section className="premium-summary-grid">
            ...
            <ReadOnlyPanel icon={Building2} title="Contexto de instalación y técnico"><dl className="premium-fields-grid"><Field label="Instalación" value={order.siteName || <EmptyState>Sin instalación</EmptyState>} /><Field label="Ubicación" value={order.locationName ? order.locationName : <span><span className="premium-pending-badge">Ubicación pendiente</span><small className="premium-subvalue">Sin ubicación</small></span>} empty={!order.locationName} /><Field label="Equipo" value={order.assetName ?? <EmptyState>Sin equipo vinculado</EmptyState>} empty={!order.assetName} /><Field label="Técnico asignado" value={order.assignedToName ? <span className="premium-technician"><span className="premium-avatar">{initials(order.assignedToName)}</span><span><strong>{order.assignedToName}</strong><small>Contacto no disponible en los datos visibles</small></span></span> : <EmptyState>Sin técnico asignado</EmptyState>} empty={!order.assignedToName} /></dl></ReadOnlyPanel>
          </section>
          ...
          <section className="premium-requirements" aria-label="Requisitos de la orden"><div className="premium-section-heading"><span className="premium-section-icon"><ShieldCheck size={18} /></span><h3>Requisitos registrados</h3></div><div className="premium-requirement-grid"><div><ListIcon checked={order.requirements.checklist} /><strong>{order.requirements.checklist ? 'Checklist requerido' : 'Sin checklist'}</strong><small>{required.length ? required.join(' · ') : 'Sin requisitos especiales'}</small></div><div><ListIcon checked={order.requirements.report} /><strong>{order.requirements.report ? 'Informe requerido' : 'Informe opcional'}</strong><small>Registro documental</small></div><div><ListIcon checked={Boolean(order.dueAt)} /><strong>{order.dueAt ? 'Fecha límite registrada' : 'Sin fecha límite'}</strong><small>{order.dueAt ? displayDate(order.dueAt) : 'No disponible'}</small></div></div></section>
          {(operationalPanels?.assignment || operationalPanels?.technical || operationalPanels?.evidence || operationalPanels?.review || operationalPanels?.cancel) && <section className="premium-operational-panels"><div className="premium-section-heading"><span className="premium-section-icon"><Wrench size={18} /></span><h3>Controles operativos existentes</h3></div>{operationalPanels.assignment}{operationalPanels.technical}{operationalPanels.evidence}{operationalPanels.review}{operationalPanels.cancel}</section>}
        </>}
        ...
        {activeTab === 'installation' && <section className="premium-installation-grid"><ReadOnlyPanel icon={Building2} title="Instalación"><dl className="premium-fields-grid"><Field label="Nombre" value={order.siteName || <EmptyState>Sin instalación</EmptyState>} /><Field label="Ubicación" value={order.locationName || <EmptyState>Sin ubicación registrada.</EmptyState>} empty={!order.locationName} /><Field label="Equipo" value={order.assetName || <EmptyState>Sin equipo vinculado</EmptyState>} empty={!order.assetName} /><Field label="Cómo llegar" value={<EmptyState>No disponible en los datos visibles.</EmptyState>} empty /></dl></ReadOnlyPanel><ReadOnlyPanel icon={MapPin} title="Acceso y contacto"><div className="premium-empty-card"><MapPin size={20} /><strong>Información pendiente</strong><p>No hay dirección, coordenadas, horario ni contacto adicional disponibles para esta instalación.</p></div></ReadOnlyPanel></section>}
        {activeTab === 'evidence' && <ReadOnlyPanel icon={FileCheck2} title="Evidencias"><div className="premium-empty-card"><FileCheck2 size={20} /><strong>Consulta informativa</strong><p>Las fotografías, checklist, informes y finalización se mantienen en los controles operativos existentes. Esta pestaña no ejecuta acciones.</p></div><div className="premium-requirement-grid premium-requirement-grid-single"><div><ListIcon checked={order.requirements.initialPhotos} /><strong>Fotografías iniciales</strong><small>{order.requirements.initialPhotos ? 'Requeridas por la OT' : 'No requeridas'}</small></div><div><ListIcon checked={order.requirements.finalPhotos} /><strong>Fotografías finales</strong><small>{order.requirements.finalPhotos ? 'Requeridas por la OT' : 'No requeridas'}</small></div><div><ListIcon checked={order.requirements.report} /><strong>Informe</strong><small>{order.requirements.report ? 'Requerido por la OT' : 'No requerido'}</small></div></div></ReadOnlyPanel>}
