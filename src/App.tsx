  ...
  return (
    <>
      ...
      <section className="dashboard-grid dashboard-grid-bottom">
        <article className="panel workload-panel"><div className="panel-heading"><h2>Carga visible por técnico</h2></div>{technicians.length === 0 ? <p className="empty-state">Sin asignaciones.</p> : <div className="workload-list">{technicians.map(([name, amount], index) => <div className="workload-row" key={name}><span className="avatar avatar-mini">{name === 'Sin asignar' ? '—' : name.split(' ').map((word) => word[0]).join('').slice(0, 2)}</span><strong>{name}</strong><div className="progress"><i style={{ width: `${Math.min(100, Math.round((amount / total) * 100))}%` }} /></div><b>{amount} OT</b><small>{Math.round((amount / total) * 100)}%</small></div>)}</div>}</article>
